...
main() {
    ...
    local media='bullseye-mmc_2g.img' # or block device '/dev/sdX'
    local deb_dist='bullseye'
    local hostname='nanopi-r5s-arm64'
    local acct_uid='debian'
    local acct_pass='debian'
    local extra_pkgs='curl, pciutils, sudo, unzip, wget, xxd, xz-utils, zip, zstd, dhcpcd5'
    ...
    local pkgs="linux-image-arm64, dbus, libpam-systemd, openssh-server, systemd-timesyncd"
